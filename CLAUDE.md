...
| `tests/` | Three jsdom suites. The README has a section on them. |

## Before calling a change finished

    npm test          # about 400 checks, roughly ten seconds
...
the real `styles.css` against a jsdom document. Add to it instead.
...
**Enharmonics are normalised by pitch class, not by string.** `Db` typed into `songs.tsv`
is stored and displayed as `C# maj`. Song ids are slugs of title plus artist, so renaming
a song in `songs.tsv` changes its id and breaks share links made before the rename.
